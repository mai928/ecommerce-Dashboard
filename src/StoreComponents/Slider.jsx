...

                {
                    E_Slider?.map((item ,index) => (
                        <SwiperSlide>
                            ...
                            <div className={`absolute inset-0   flex flex-col ${E_Slider[0]=== E_Slider[index]?'justify-center items-center ':'items-center lg:items-start justify-center text-center lg:ms-32 text-white'} `}>
                                
                                <p className={`  font-semibold text-lg ${index === 0? 'text-white text-center':'text-yellow-600'}`}>{item.label}</p>
                                <h2 className={`text-2xl lg:text-6xl font-semibold     ${index ===0 ?' font-pacifico w-[600px] text-e_secondaryColor text-center my-10':'w-[600px] text-center  font-pacifico lg:font-sans my-10 font-bold lg:text-start'}`}>{item.title}</h2>
